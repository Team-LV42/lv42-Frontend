...
const AddableItem = ({index, btnList, btnMalfTypeList }) => {
	...
	return (
		<>
		<div key={index} className="relative flex flex-row items-center justify-center w-full p-2 button-active">
			<button
			onClick={() => setSelectBtnisVisible((prev) => !prev)}
			className="group md:text-lg flex flex-row items-center justify-around text-center md:w-20 w-12 mx-1 md:h-12 h-9 border border-black pointerhover:hover:bg-gray-300 rounded p-2"
			>
				<p className="inline">{btnName(content.controller_btn_id)}</p>
				<div className={`${selectBtnisVisible ? 'visible' : 'invisible'} grid gap-2 grid-rows-4 grid-cols-5 p-4 absolute md:top-[3.6rem] top-11 left-0 w-full md:h-60 h-44 bg-gray-200 border border-black rounded-md z-50 shadow-md`}>
			...
			>
				...
				<div className={`${selectBtnMalfunisVisible ? 'visible' : 'invisible'} flex-col items-center justify-center p-4 absolute md:top-[3.6rem] top-11 left-0 w-full bg-gray-200 border border-black rounded-md z-50 shadow-md`}>
					...
					onBlur={(e) => {
						...
						setSelectBtnMalfunisVisible((prev) => !prev);
					}}
